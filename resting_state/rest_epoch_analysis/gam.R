...
# Load data
df <- read_excel("C:\\Users\\Olle de Jong\\Desktop\\data_mim_mic.xlsx")
...
# Main function
make_gams <- function(df, state, gam_formula, max_freq, experiment_name,
                      output_path) {
  # Subset data
  ...
  # Export Predictions
  write_xlsx(predAct, sprintf("%s/pred_%s_%s.xlsx", output_path, experiment_name, state))
  
  # Summary Sink
  sink(sprintf("%s/summary_%s_%s.txt", output_path, experiment_name, state))
  summary(g1)
  save_plots(g1, output_path, experiment_name, state, max_freq)
  sink()
...
experiment_name <- 'resting_state'
states <- c("rest")
max_freq <- 100
output_path <- 'C:/Users/Olle de Jong/Documents/MSc Biology/rp2/rp2_data/resting_state/output/gams'
...
# Loop over behaviors
for (state in states) {
  sprintf("Working on %s events", state)
  g1 <- make_gams(df, state, as.formula(gam_formula), max_freq, experiment_name, output_path)
}
