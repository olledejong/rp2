...
save_plots <- function(g1, output_path, channel, experiment_name, event_type) {
  ...
  # also save the estimations where the random effects are still in there
  pdf(sprintf("%s/est_power_w_random_%s_%s_%s.pdf", output_path, experiment_name, event_type, channel), width = 10, height = 8)
  plot_smooth(
    g1, view = "freq", rug = F, plot_all = "genotype", 
    main = sprintf("Modelled power data (%s, %s events)", chan_names[[channel]], gsub("_", " ", event_type)),
    xlab = "Frequency (Hz)", ylab = "Estimated power (dB/Hz)",
    col = c('#EB5E55', '#419D78'), lwd = 2.5, rm.ranef = FALSE
  )
  dev.off()
}
...
# ____________ GAMs for single event type (Adjust the gam_formula) _____________
#_______________________________________________________________________________
# Define these settings
event_type <- 'social_cup'
max_freq <- 100
experiment_name <- '3_chamber_sociability'
output_path <- 'MSc Biology/rp2/rp2_data/3C_sociability/output/gams'
